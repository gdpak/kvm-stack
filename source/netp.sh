...
dnsmasq --strict-order --except-interface=lo --interface=br0 --listen-address=192.168.100.1 --bind-interfaces --dhcp-range=192.168.100.100,192.168.100.200,30d --conf-file= --no-hosts --addn-hosts=/tmp/puppet.hosts --pid-file=/var/run/qemu-dnsmasq-br0.pid --dhcp-leasefile=/var/run/qemu-dnsmasq-br0.leases --dhcp-no-override --dhcp-fqdn --domain=domain.name --dhcp-hostsfile=/tmp/puppet.hostfile
if [ $? -ne 0 ]
then
...
dnsmasq --strict-order --except-interface=lo --interface=br1 --listen-address=172.24.133.1 --bind-interfaces --dhcp-range=172.24.133.100,172.24.133.200,30d --conf-file= --pid-file=/var/run/qemu-dnsmasq-br1.pid --dhcp-leasefile=/var/run/qemu-dnsmasq-br1.leases --dhcp-no-override
if [ $? -ne 0 ]
then
